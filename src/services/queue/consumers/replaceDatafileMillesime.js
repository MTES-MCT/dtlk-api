...
let replaceDatafileMillesimeConsumer =  queue.process('replaceDatafileMillesime', 1 , async function (job, done) {
  let notify = (progress, total, data) => job.progress(progress, total, data)
  let tempCollectionNameMongo = `${ job.data.ridDatafile }-updateMillesime-${ job.data.millesimeDatafile }`
  let result = {
    success: false,
    finish: false,
    progress: 0,
  ...
  // processing job
  try {
    job.progress(0, 100, commons.result.update(result, { step: `Début du traitement du fichier ${ job.data.nameFile } - Remplacement du millésime ${ job.data.millesimeDatafile } du fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }` }))
    ...
    let datafile = await commons.retrieve.datafile(result.dataset, job.data.ridDatafile)
    job.progress(0, 100, commons.result.update(result, { datafile: datafile })) // => result.datafile = { rid, title, description, millesimes, millesimes_info: { millesime, rows, columns }, url }
    ...
    job.progress(95, 100, commons.result.update(result, { step: `Remplacement du millésime ${ job.data.millesimeDatafile } du fichier de données dans udata - démarrage` }))
    datafile = await commons.udata.updateDatafileMillesime(result.user.apiKey, job.data.idDataset, job.data.ridDatafile, job.data.millesimeDatafile, result.split.totalLines, result.columns, job.data.date_diffusion, job.data.heure_diffusion)
    job.progress(96, 100, commons.result.update(result, { step: `Remplacement du millésime ${ job.data.millesimeDatafile } du fichier de données dans udata - fin`, progress: 96, datafile: datafile })) // => result.datafile = { rid, title, description, millesimes, millesimes_info: { millesime, rows, columns }, url }
    ...
    job.progress(100, 100, commons.result.update(result, { step: `Fin du traitement du fichier ${ job.data.nameFile } - Remplacement du millésime ${ job.data.millesimeDatafile } du fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }`, success: true, finish: true }))

    await commons.logs.replaceDatafileMillesime(result.user, result.dataset, result.datafile, job.data.millesimeDatafile)
  }
  catch (error) {
    let err = { message: error.message }
    if (error.errorsList) err.csvParsingErrorsList = error.errorsList
    job.progress(100, 100, commons.result.update(result, { step: `Erreur du traitement du fichier ${ job.data.nameFile } - Remplacement du millésime ${ job.data.millesimeDatafile } du fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }`, success: false, finish: true, error: err }))
  }
...
module.exports = replaceDatafileMillesimeConsumer
