...
let addDatafileMillesimeConsumer =  queue.process('addDatafileMillesime', 1 , async function (job, done) {
  let notify = (progress, total, data) => job.progress(progress, total, data)
  let tempCollectionNameMongo = `${ job.data.ridDatafile }-newMillesime`
  let result = {
    success: false,
    finish: false,
    progress: 0,
  ...
  // processing job
  try {
    job.progress(0, 100, commons.result.update(result, { step: `Début du traitement du fichier ${ job.data.nameFile } - Ajout d'un millésime au fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }` }))
    ...
    job.progress(95, 100, commons.result.update(result, { step: `Ajout du millésime ${ result.datafile.millesimes + 1 } au fichier de données dans udata - démarrage` }))
    datafile = await commons.udata.addDatafileMillesime(result.user.apiKey, job.data.idDataset, job.data.ridDatafile, result.split.totalLines, result.columns)
    job.progress(96, 100, commons.result.update(result, { step: `Ajout du millésime ${ result.datafile.millesimes } au fichier de données dans udata - fin`, progress: 96, datafile: datafile })) // => result.datafile = { rid, title, description, millesimes, millesimes_info: { millesime, rows, columns }, url }
    ...
    job.progress(100, 100, commons.result.update(result, { step: `Fin du traitement du fichier ${ job.data.nameFile } - Ajout d'une millésime au fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }`, success: true, finish: true }))

    await commons.logs.addDatafileMillesime(result.user, result.dataset, result.datafile)
  }
  catch (error) {
    console.log(error)
    let err = { message: error.message }
    if (error.errorsList) err.csvParsingErrorsList = error.errorsList
    job.progress(100, 100, commons.result.update(result, { step: `Erreur traitement du fichier ${ job.data.nameFile } - Ajout d'un millésime au fichier de données ${ job.data.ridDatafile } du jeu de données ${ job.data.idDataset }`, success: false, finish: true, error: err }))
  }
...
module.exports = addDatafileMillesimeConsumer
